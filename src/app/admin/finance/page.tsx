...
import FinanceSelect from '@/components/admin/finance/atoms/FinanceSelect';
import { FinanceChannelCard } from '@/components/admin/finance/atoms/FinanceChannelCard';
import { KpiCard } from '@/components/admin/finance/atoms/KpiCard';
import { FinanceRangeCard } from '@/components/admin/finance/atoms/FinanceRangeCard';
...
export type AccountType = 'cash' | 'bank' | 'pos' | 'wallet';
...
type ROISimulatorProps = {
  ...
  setSalesIncrease: (value: number) => void;
  result: CampaignROI;
  formatCurrency: (value: number) => string;
};

type InventoryFinancialImpactProps = {
  totalCost: number;
  potentialRevenue: number;
  avgMargin: number;
  formatCurrency: (value: number) => string;
...
// ============================================================
const BudgetVsActual = ({ budgetData, onEditBudget }: BudgetVsActualProps) => {
...
const CashFlowForecast = ({ data, formatCurrency }: CashFlowForecastProps) => (
  <div className="p-5 rounded-2xl bg-white shadow-sm border border-slate-200">
    ...
    <div className="h-48"><ResponsiveContainer width="100%" height="100%"><LineChart data={data.slice(0,30)}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip formatter={(value) => formatCurrency(Number(value) || 0)} /><Legend /><Line type="monotone" dataKey="cumulativeCash" name="Kumulyativ nakit" stroke="#22c55e" strokeWidth={2} /></LineChart></ResponsiveContainer></div>
  </div>
);

const FinancialHealthScore = ({ score, metrics }: FinancialHealthScoreProps) => (
  <div className="p-5 rounded-2xl bg-white shadow-sm border border-slate-200">
...
const ROISimulator = ({ discount, setDiscount, salesIncrease, setSalesIncrease, result, formatCurrency }: ROISimulatorProps) => (
...
const InventoryFinancialImpact = ({ totalCost, potentialRevenue, avgMargin, formatCurrency }: InventoryFinancialImpactProps) => (
  <div className="p-5 rounded-2xl bg-white shadow-sm border border-slate-200">
...
const WarningPanel = ({ warnings }: WarningPanelProps) => (
...
export default function FinancePage() {
  ...
  const [accountForm, setAccountForm] = useState({
    name: '',
    type: 'cash' as 'cash' | 'bank' | 'pos' | 'wallet',
    currency: 'AZN',
  });
  ...
  // ========== ƏSAS HESABLAMALAR ==========
  const revenue = useMemo(() => orders.reduce((sum, o) => sum + o.items.reduce((s, it) => s + Number(it.priceAtOrder ?? 0) * (it.qty || 0), 0), 0), [orders]);
  const totalExpenses = finance.totalExpenses();
  const totalCogs = finance.totalCOGS();
  ...
  const computeRange = useCallback((days: number): RangeStats => {
    ...
    const income = rOrders.reduce((s, o) => s + o.items.reduce((x, it) => x + Number(it.priceAtOrder ?? 0) * (it.qty || 0), 0), 0);
    const exp = finance.expenses.filter(e => inLastDays(e.date, days)).reduce((s, e) => s + e.amount, 0);
    const profit = income - exp;
    const margin = income > 0 ? (profit / income) * 100 : 0;
    return { income, exp, profit, margin };
  }, [orders, finance.expenses]);
  ...
  const monthExpenses = useMemo(() => finance.expenses.filter(e => e.date.startsWith(monthKey)), [finance.expenses, monthKey]);
  const monthlyStats: RangeStats = useMemo(() => {
    const income = monthOrders.reduce((s, o) => s + o.items.reduce((x, it) => x + Number(it.priceAtOrder ?? 0) * (it.qty || 0), 0), 0);
  ...
  const cashFlowProjection: CashFlowProjection[] = useMemo(() => {
    ...
    for (const o of orders) {
      ...
      const amount = o.items.reduce((s, it) => s + Number(it.priceAtOrder ?? 0) * (it.qty || 0), 0);
    ...
    for (let i = 1; i <= 30; i++) {
      const date = new Date(initialTimestamp + i * DAY_MS).toISOString().slice(0, 10);
  ...
  const topProducts: TopProductRow[] = useMemo(() => {
    ...
    for (const o of orders) {
      for (const it of o.items) {
        ...
        const cost = Number(it.costAtOrder ?? 0);
        const qty = it.qty ?? 0;
  ...
  const chartData = useMemo(() => {
    ...
    for (const o of orders) {
      ...
      const sales = o.items.reduce((x, it) => x + Number(it.priceAtOrder ?? 0) * (it.qty || 0), 0);
  ...
  const expensePie = useMemo(() => {
    ...
    for (const e of finance.expenses) catMap.set(e.category, (catMap.get(e.category) || 0) + e.amount);
    return Array.from(catMap.entries()).map(([name, value]) => ({ name, value }));
  }, [finance.expenses]);

  const aiInsight = useMemo(() => generateAiInsight({ monthLabel: currentMonthLabel, monthlyStats, expensePie, channelTotals, logs: monthLogs }), [currentMonthLabel, monthlyStats, expensePie, channelTotals, monthLogs]);
  ...
  const addSupplier = () => {
    ...
    finance.addSupplier(supplierForm);
    setSupplierForm({
      name: '',
      phone: '',
      email: '',
  ...
  const addAccount = () => {
    ...
    finance.addAccount(accountForm);
    setAccountForm({
      name: '',
      type: 'cash',
      currency: 'AZN',
  ...
  const addPurchase = () => {
    if (!purchaseForm.supplierId || !purchaseForm.productId || purchaseForm.qty <= 0) return;
    finance.addPurchase({
  ...
  const addPayment = () => {
    if (!paymentForm.supplierId || !paymentForm.accountId || paymentForm.amount <= 0) return;
    finance.addPayment({
  ...
  const addDailyLog = () => {
    ...
    const totalExpensesForDay = finance.expenses.filter(e => e.date === dailyForm.date).reduce((s, e) => s + e.amount, 0);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-emerald-50 via-lime-50 to-amber-50 p-4 md:p-8 space-y-10">
      ...
      <section className="grid xl:grid-cols-[1.3fr_1.4fr] gap-6">
        <div className="p-5 border border-slate-200 rounded-2xl bg-white shadow-sm">
          <div className="flex justify-between items-center mb-3"><h2 className="text-lg font-semibold text-slate-900">Xərclər Cədvəli</h2><span className="text-xs bg-slate-50 px-3 py-1 rounded-full">{finance.expenses.length} xərc · {formatCurrency(totalExpenses)}</span></div>
          <div className="max-h-[320px] overflow-y-auto border border-slate-100 rounded-xl">
            <table className="w-full text-xs md:text-sm">
              ...
              <tbody>
                ...
                {finance.expenses.length === 0 && <tr><td colSpan={4} className="px-3 py-4 text-center text-xs text-slate-500">Hələ xərc əlavə edilməyib.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        <div className="space-y-5">
          <div className="p-5 border border-emerald-100 rounded-2xl bg-white shadow">
            ...
            <div className="h-64"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip formatter={(value) => formatCurrency(Number(value) || 0)} /><Legend /><Line type="monotone" dataKey="sales" name="Satış" stroke="#22c55e" strokeWidth={2.2} dot={false} /><Line type="monotone" dataKey="expenses" name="Xərc" stroke="#ef4444" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="profit" name="Mənfəət" stroke="#0ea5e9" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div>
          </div>
          <div className="p-5 border border-slate-200 rounded-2xl bg-white shadow">
            ...
            <div className="h-64 flex items-center justify-center">
              {expensePie.length ? (
                <ResponsiveContainer><RePieChart><Pie data={expensePie} cx="50%" cy="50%" outerRadius={100} dataKey="value" label={({ name, percent }) => `${name} (${((percent ?? 0) * 100).toFixed(0)}%)`}>{expensePie.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}</Pie><Tooltip formatter={(value) => formatCurrency(Number(value) || 0)} /></RePieChart></ResponsiveContainer>
              ) : (
      ...
      <section className="grid lg:grid-cols-2 gap-6">
        <div className="p-5 border border-slate-200 rounded-2xl bg-white shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold text-slate-900 flex items-center gap-2"><PackageSearch className="w-5 h-5 text-emerald-600" />Tədarükçülər</h2>
            <button onClick={() => setShowSupplierModal(true)} className="text-xs bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg hover:bg-emerald-100 font-semibold">+ Yeni tədarükçi</button>
          </div>
          <div className="max-h-[300px] overflow-y-auto border border-slate-100 rounded-xl">
            <table className="w-full text-xs md:text-sm">
              <thead className="bg-slate-50 sticky top-0"><tr><th className="p-2">Ad</th><th>Telefon</th><th>Ödəniş müddəti</th><th>Borc</th></tr></thead>
              <tbody>
                {finance.suppliers.map(s => {
                  const ap = finance.apSnapshot().find(x => x.supplierId === s.id);
                  return (
                    <tr key={s.id} className="border-b hover:bg-emerald-50/40">
                      <td className="p-2 font-semibold">{s.name}</td>
                      <td className="p-2">{s.phone || '—'}</td>
                      <td className="p-2">{s.paymentTermDays ?? 7} gün</td>
                      <td className={`p-2 font-semibold ${ap?.amount ? 'text-rose-600' : 'text-emerald-600'}`}>{formatCurrency(ap?.amount || 0)}</td>
                    </tr>
                  );
                })}
                {finance.suppliers.length === 0 && <tr><td colSpan={4} className="px-3 py-4 text-center text-xs text-slate-500">Hələ tədarükçi əlavə edilməyib.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="p-5 border border-slate-200 rounded-2xl bg-white shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold text-slate-900 flex items-center gap-2"><Wallet className="w-5 h-5 text-emerald-600" />Hesablar</h2>
            <button onClick={() => setShowAccountModal(true)} className="text-xs bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg hover:bg-emerald-100 font-semibold">+ Yeni hesab</button>
          </div>
          <div className="max-h-[300px] overflow-y-auto border border-slate-100 rounded-xl">
            <table className="w-full text-xs md:text-sm">
              <thead className="bg-slate-50 sticky top-0"><tr><th className="p-2">Ad</th><th>Növ</th><th>Balans</th></tr></thead>
              <tbody>
                {finance.cashBalances().map(a => (
                  <tr key={a.id} className="border-b hover:bg-emerald-50/40">
                    ...
                    <td className={`p-2 font-semibold ${a.balance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{formatCurrency(a.balance)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="grid lg:grid-cols-2 gap-6">
        <div className="p-5 border border-slate-200 rounded-2xl bg-white shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold text-slate-900 flex items-center gap-2"><ShoppingBag className="w-5 h-5 text-emerald-600" />Alışlar</h2>
            <button onClick={() => setShowPurchaseModal(true)} className="text-xs bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg hover:bg-emerald-100 font-semibold">+ Yeni alış</button>
          </div>
          <div className="max-h-[300px] overflow-y-auto border border-slate-100 rounded-xl">
            <table className="w-full text-xs md:text-sm">
              <thead className="bg-slate-50 sticky top-0"><tr><th className="p-2">Tarix</th><th>Məhsul</th><th>Miqdar</th><th>Maya</th><th>Cəmi</th></tr></thead>
              <tbody>
                {finance.purchases.map(p => {
                  const product = safeProducts.find(x => x.id === p.productId);
                  return (
                    <tr key={p.id} className="border-b hover:bg-emerald-50/40">
                      <td className="p-2">{new Date(p.date).toLocaleDateString('az-AZ')}</td>
                      <td className="p-2">{product?.name || 'Naməlum'}</td>
                      <td className="p-2">{p.qty}</td>
                      <td className="p-2">{formatCurrency(p.unitCost)}</td>
                      <td className="p-2 text-emerald-700 font-semibold">{formatCurrency(p.qty * p.unitCost)}</td>
                    </tr>
                  );
                })}
                {finance.purchases.length === 0 && <tr><td colSpan={5} className="px-3 py-4 text-center text-xs text-slate-500">Hələ alış əlavə edilməyib.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="p-5 border border-slate-200 rounded-2xl bg-white shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold text-slate-900 flex items-center gap-2"><CreditCard className="w-5 h-5 text-emerald-600" />Ödənişlər</h2>
            <button onClick={() => setShowPaymentModal(true)} className="text-xs bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg hover:bg-emerald-100 font-semibold">+ Yeni ödəniş</button>
          </div>
          <div className="max-h-[300px] overflow-y-auto border border-slate-100 rounded-xl">
            <table className="w-full text-xs md:text-sm">
              <thead className="bg-slate-50 sticky top-0"><tr><th className="p-2">Tarix</th><th>Tədarükçi</th><th>Hesab</th><th>Məbləğ</th></tr></thead>
              <tbody>
                {finance.payments.map(p => {
                  const supplier = finance.suppliers.find(s => s.id === p.supplierId);
                  const account = finance.accounts.find(a => a.id === p.accountId);
                  return (
                    <tr key={p.id} className="border-b hover:bg-emerald-50/40">
                      <td className="p-2">{new Date(p.date).toLocaleDateString('az-AZ')}</td>
                      <td className="p-2">{supplier?.name || 'Naməlum'}</td>
                      <td className="p-2">{account?.name || 'Naməlum'}</td>
                      <td className="p-2 text-emerald-700 font-semibold">{formatCurrency(p.amount)}</td>
                    </tr>
                  );
                })}
                {finance.payments.length === 0 && <tr><td colSpan={4} className="px-3 py-4 text-center text-xs text-slate-500">Hələ ödəniş əlavə edilməyib.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {showSupplierModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl m-4">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Yeni Tədarükçi</h2>
            <div className="space-y-3">
              <Input label="Ad" value={supplierForm.name} onChange={(val: string) => setSupplierForm({ ...supplierForm, name: val })} />
              <Input label="Telefon" value={supplierForm.phone} onChange={(val: string) => setSupplierForm({ ...supplierForm, phone: val })} />
              <Input label="Email" value={supplierForm.email} onChange={(val: string) => setSupplierForm({ ...supplierForm, email: val })} />
              <Input label="Ünvan" value={supplierForm.address} onChange={(val: string) => setSupplierForm({ ...supplierForm, address: val })} />
              <Input label="Ödəniş müddəti (gün)" type="number" value={supplierForm.paymentTermDays} onChange={(val: string) => setSupplierForm({ ...supplierForm, paymentTermDays: Number(val) || 7 })} />
              <Input label="Qeyd" value={supplierForm.note} onChange={(val: string) => setSupplierForm({ ...supplierForm, note: val })} />
            </div>
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setShowSupplierModal(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50">Ləğv et</button>
              <button onClick={addSupplier} className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700">Yarat</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAccountModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl m-4">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Yeni Hesab</h2>
            <div className="space-y-3">
              <Input label="Ad" value={accountForm.name} onChange={(val: string) => setAccountForm({ ...accountForm, name: val })} />
              <FinanceSelect label="Növ" value={accountForm.type} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setAccountForm({ ...accountForm, type: e.target.value as AccountType })}>
            ...
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setShowAccountModal(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50">Ləğv et</button>
              <button onClick={addAccount} className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700">Yarat</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPurchaseModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl m-4 max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Yeni Alış</h2>
            <div className="space-y-3">
              <Input label="Tarix" type="date" value={purchaseForm.date} onChange={(val: string) => setPurchaseForm({ ...purchaseForm, date: val })} />
              <FinanceSelect label="Tədarükçi" value={purchaseForm.supplierId} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setPurchaseForm({ ...purchaseForm, supplierId: e.target.value })}>
                <option value="">Seçin</option>
                {finance.suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </FinanceSelect>
              <FinanceSelect label="Məhsul" value={purchaseForm.productId} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setPurchaseForm({ ...purchaseForm, productId: e.target.value })}>
                <option value="">Seçin</option>
                {safeProducts.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
              </FinanceSelect>
              <div className="grid grid-cols-2 gap-3">
                <Input label="Miqdar" type="number" value={purchaseForm.qty} onChange={(val: string) => setPurchaseForm({ ...purchaseForm, qty: Number(val) || 0 })} />
                <Input label="Maya (₼)" type="number" value={purchaseForm.unitCost} onChange={(val: string) => setPurchaseForm({ ...purchaseForm, unitCost: Number(val) || 0 })} />
              </div>
              <FinanceSelect label="Hesab" value={purchaseForm.accountId} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setPurchaseForm({ ...purchaseForm, accountId: e.target.value })}>
                <option value="">Seçin</option>
                {finance.accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
              </FinanceSelect>
              <Input label="Dərhal ödənilən (₼)" type="number" value={purchaseForm.paid} onChange={(val: string) => setPurchaseForm({ ...purchaseForm, paid: Number(val) || 0 })} />
              <Input label="Qeyd" value={purchaseForm.note} onChange={(val: string) => setPurchaseForm({ ...purchaseForm, note: val })} />
            </div>
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setShowPurchaseModal(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50">Ləğv et</button>
              <button onClick={addPurchase} className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700">Yarat</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl m-4">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Yeni Ödəniş</h2>
            <div className="space-y-3">
              <Input label="Tarix" type="date" value={paymentForm.date} onChange={(val: string) => setPaymentForm({ ...paymentForm, date: val })} />
              <FinanceSelect label="Tədarükçi" value={paymentForm.supplierId} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setPaymentForm({ ...paymentForm, supplierId: e.target.value })}>
                <option value="">Seçin</option>
                {finance.suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </FinanceSelect>
              <FinanceSelect label="Hesab" value={paymentForm.accountId} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setPaymentForm({ ...paymentForm, accountId: e.target.value })}>
                <option value="">Seçin</option>
                {finance.accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
              </FinanceSelect>
              <Input label="Məbləğ (₼)" type="number" value={paymentForm.amount} onChange={(val: string) => setPaymentForm({ ...paymentForm, amount: Number(val) || 0 })} />
              <Input label="Qeyd" value={paymentForm.note} onChange={(val: string) => setPaymentForm({ ...paymentForm, note: val })} />
            </div>
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setShowPaymentModal(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50">Ləğv et</button>
              <button onClick={addPayment} className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700">Yarat</button>
            </div>
          </div>
        </div>
      )}
