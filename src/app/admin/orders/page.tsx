...
export default function OrdersPage() {
  ...
  const getProductName = useCallback(
    (item: any) => {
      return item.productName || "Məhsul";
    },
    [],
  );
  // Transform orders from DB to frontend format
  const ordersWithTotals = useMemo<OrderFull[]>(() => {
    if (!hasHydrated) return [];

    return orders.map((o: any) => transformOrderForFrontend(o));
  }, [orders, hasHydrated]);
  ...
  const { metrics, timeWindowStats, paymentStats } = useOrderAnalytics(
    ordersWithTotals,
    filteredOrders,
    () => ({ totalRevenue: 0, totalOrders: 0 }), // Placeholder analytics function
  );
  ...
  const handleStatusChange = useCallback(
    (orderId: ID, newStatus: string) => {
      ...
      // Convert display status to DB status
      const dbStatus = fromDisplayStatus(newStatus as OrderStatusDisplay);
      updateOrderStatusMutation.mutate({ id: orderId, status: dbStatus });

      setSelectedOrder((prev) =>
        prev && prev.id === orderId
          ? { ...prev, status: newStatus as OrderStatusDisplay }
          : prev,
      );
    },
  ...
  const confirmCancel = () => {
    if (orderToCancel) {
      updateOrderStatusMutation.mutate({ id: orderToCancel, status: "CANCELLED" });
    }
  ...
  const handleBulkStatusChange = () => {
    ...
    selectedOrderIds.forEach((id) =>
      updateOrderStatusMutation.mutate({ id, status: dbStatus }),
    );
  ...
  />
      ...
      <OrdersFilterBar
        ...
        onChangeStatus={(v) => {
          setFilterStatus(v as OrderStatusDisplay);
          setPage(1);
        }}
      ...
      {selectedOrderIds.length > 0 && (
        <OrdersBulkActionsBar
          ...
          onBulkStatusChange={(v) => {
            setBulkUpdateStatus(v as OrderStatusDisplay);
          }}
