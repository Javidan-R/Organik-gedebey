...
function calcMargin(sale: number, cost: number): number {
  if (sale <= 0) return 0;
...
function ProductRow({
  ...
  const [activeTab, setActiveTab] = useState<"general" | "price" | "deal">(
    "general"
  );
  ...
  const baseStock = getTotalStock(product);
        ...
        <div className="flex items-center gap-2.5 p-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {isFresh && (
                <span
                  className="text-[9px] font-bold bg-emerald-100 text-emerald-700
              ...
              {isUpcoming && (
                <span
                  className="text-[9px] font-bold bg-amber-100 text-amber-700
              ...
              {flashDeal && (
                <span
                  className="text-[9px] font-bold bg-yellow-100 text-amber-800
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              {discountType !== "none" && discountValue > 0 && (
                <span className="text-[10px] text-amber-700 font-bold bg-yellow-100 px-1.5 rounded-full">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button
              ...
              title={
                isFresh
                  ? "Aktiv siyahıdan çıxar"
                  : "Aktiv siyahıya əlavə et"
              }
            ...
            <button
              ...
              title={
                isUpcoming
                  ? "Gəlir siyahısından çıxar"
                  : "Gəlir siyahısına əlavə et"
              }
              className={`w-8 h-8 rounded-lg flex items-center justify-center
                transition-colors ${
                isUpcoming
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setLocalSalePrice(parseFloat(e.target.value) || 0)
                        }
                        className="w-full text-sm border border-slate-200 rounded-xl px-3 py-2
                          outline-none focus:border-emerald-400 bg-white
                          focus:ring-2 focus:ring-emerald-100 transition-all"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setLocalCostPrice(parseFloat(e.target.value) || 0)
                        }
                        className="w-full text-sm border border-slate-200 rounded-xl px-3 py-2
                          outline-none focus:border-emerald-400 bg-white
                          focus:ring-2 focus:ring-emerald-100 transition-all"
                    ...
                    <div>
                      <SectionLabel>
                        <BadgePercent className="w-3 h-3 inline mr-0.5" />
                        Endirim növü
                      </SectionLabel>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() =>
                            setLocalStock((s) => Math.max(0, s - 1))
                          }
                    ...
                    {(product.variants?.length ?? 0) > 1 && (
                      <label className="flex items-center gap-2 cursor-pointer
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setApplyToAllVariants(e.target.checked)
                          }
...
export default function AdminFreshTodayPage() {
  ...
  const filteredList = useMemo(() => {
    ...
    if (sortBy === "stock-asc")
      sorted.sort(
        (a, b) =>
          getTotalStock(a) - getTotalStock(b)
      );
    else if (sortBy === "stock-desc")
      sorted.sort(
        (a, b) =>
          getTotalStock(b) - getTotalStock(a)
      );
      ...
      sorted.sort((a, b) => {
        const mA = calcMargin(
          ...
          a.variants?.[0]?.costPrice ?? a.costPrice ?? 0
        );
        const mB = calcMargin(
          b.variants?.[0]?.price ?? b.price ?? 0,
          b.variants?.[0]?.costPrice ?? b.costPrice ?? 0
        );
        return mB - mA;
      });
  ...
  const outCount = freshList.filter((p) => getTotalStock(p) <= 0).length;
  ...
  const selectAll = () => setSelectedIds(new Set(filteredList.map((p) => p.id)));
  ...
  /* Toggle helpers */
  const handleToggleFresh = useCallback(
    (id: string) => {
  ...
  const handleUpdate = useCallback(
    (id: string, partial: Partial<ManagedProduct>) => {
      const p = managedProducts.find((x) => x.id === id);
      if (!p) return;
    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 pt-4 pb-2">
          <div
            className="w-10 h-10 rounded-2xl bg-emerald-700
      ...
      <div className="p-4 space-y-2">
        <AnimatePresence mode="popLayout">
          {filteredList.length === 0 ? (
            ...
            >
              <div
                className="w-16 h-16 bg-white rounded-3xl shadow-sm
