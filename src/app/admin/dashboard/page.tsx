...
export default function AdminDashboardPage() {
  ...
  const discountEffectiveness = useMemo(() => {
    ...
    const discountedRevenue = orders
      ...
      .reduce((s, it) => s + Number(it.priceAtOrder) * it.qty, 0)
      .toFixed(2);
  ...
  const monthlySales = useMemo(() => {
    ...
    const chartData: Array<{ month: string; historical: number | null; forecast: number | null }> = arr.map((item) => ({
      month: item.month,
      historical: item.qty,
    ...
    const totalRevenue = orders
      ...
      .reduce(
        (s, o) =>
          s +
          o.items.reduce(
            (sum, it) => sum + it.qty * Number(it.priceAtOrder),
            0,
          ),
        0,
      );
