...
export const HomePageClient: FC<HomePageClientProps> = ({ initialData }) => {
  ...
  useEffect(() => {
    ...
    if (orders.length === 0) setOrders(initialData.orders);
  }, [initialData, products.length, categories.length, orders.length, setProducts, setCategories, setOrders]);
  ...
  const heroHighlighted = useMemo((): Product | null => {
    ...
    const viewedIds = recentViewed.map((p) => p.id);
    return [...products.filter((p) => !p.archived)].sort(
            ...
            >
              <SectionBlock id="flash-deal" title="⚡ Anlıq Fırsat" subtitle="Məhdud vaxtlı premium deal" badge="🔥 HOT">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FlashDealCard {...flashDeal} timeLeft={formatTimer(secondsLeft)} />
