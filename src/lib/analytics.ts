...
export const revenueSeries = (orders: Order[], _products: Product[], g: Grouping = 'day') => {
  const map = new Map<string, number>()
  for (const o of orders) {
    const key = fmt(o.createdAt, g)
    const sum = o.items.reduce((s: number, it: OrderItem) => s + (Number(it.priceAtOrder) * it.qty), 0)
  ...
  for (const o of orders) {
    for (const it of o.items) {
      ...
      if (catId !== null && catId !== undefined) {
        catMap.set(catId, (catMap.get(catId) || 0) + Number(it.priceAtOrder) * it.qty)
      }
    }
  }
...
export const aovSeries = (orders: Order[], g: Grouping = 'day') => {
  ...
  for (const o of orders) {
    ...
    const total = o.items.reduce((s: number, it: OrderItem) => s + Number(it.priceAtOrder) * it.qty, 0)
...
export const quickSummary = (orders: Order[]) => {
  ...
  const sumBy = (pred: (o: Order) => boolean) =>
    orders.filter(pred).reduce((s: number, o: Order) => s + o.items.reduce((x: number, it: OrderItem) => x + Number(it.priceAtOrder) * it.qty, 0), 0)
